...
function createParticles(scene, type) {
    ...
    if (type === 'uncommon') {
        const emitters = particles.map(p => p.createEmitter({
            ...
            lifespan: 5000,
            x: {
                min: scene.sys.game.config.width / 2 - 25,
                max: scene.sys.game.config.width / 2 + 25
            },
    ...
    // --- Rare ---
        ...
        for (let i = 0; i < 7; i++) {
            ...
            emitters.push(particles[i].createEmitter({
                ...
                lifespan: 5000,
                x: {
                    min: scene.sys.game.config.width - 25,
                    max: scene.sys.game.config.width + 25
                },
                y: scene.sys.game.config.height,
                rotate: {
                    onEmit: () => 0,
    ...
    // --- Epic ---
        ...
        for (let i = 0; i < 7; i++) {
            ...
            particles[i].createEmitter({
                ...
                y: {
                    min: 0,
                    max: scene.sys.game.config.height
                },
                rotate: {
                    onEmit: () => 0,
                    onUpdate: p => p.angle + 1
    ...
    // --- Legendary ---
        ...
        for (let i = 0; i < 7; i++) {
            particles[i].createEmitter({
                ...
                x: {
                    min: 0,
                    max: scene.sys.game.config.width
                },
                y: -50,
                rotate: {
                    onEmit: () => 0,
